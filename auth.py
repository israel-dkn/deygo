...
class Orders(db.Model):
    ...
    test_col = db.Column(db.String) #if rating provided
...
class PartnerLoginForm(FlaskForm):
    ...
    email = StringField('Email', validators=[DataRequired()])
    password = PasswordField('Password', validators=[DataRequired()])
    submit = SubmitField("Login Partner")
...
@app.route("/rsuper-admin", methods=["POST", "GET"], defaults={"page" : 1, "query":None})
@app.route("/rsuper-admin/<query>/<int:page>", methods=["POST", "GET"])
def deygo_admin(query, page):
    print(query)
    # if request.method == 'POST' and 'tag' in request.form:
    #     tag = request.form["tag"]
    #     search = "%{}%".format(tag)
    #     partners = PartnerSignup.query.filter(PartnerSignup.username.like(search)).paginate(per_page=10, error_out=True) # OR: from sqlalchemy import or_  filter(or_(User.name == 'ednalan', User.name == 'caite'))
    #     return render_template('super-admin.html', 
    #                            partners=partners, 
    #                            tag=tag)
    users = UserSignup.query.order_by(UserSignup.id).paginate(page=page if query=='users' else 1, per_page=10)
    partners = PartnerSignup.query.order_by(PartnerSignup.id).paginate(page=page if query=='partner' else 1, per_page=10)
    sitevisits = SiteVisits.query.order_by(SiteVisits.id).paginate(page=page if query=='sitevisits' else 1, per_page=10)
    wallets = MyWallet.query.order_by(MyWallet.id).paginate(page=page if query=='wallet' else 1, per_page=10)
    orders = Orders.query.order_by(Orders.id).paginate(page=page if query=='orders' else 1, per_page=10)
...
def arrived(id):
    ...
    if orders.status == "completed":
        flash("Request has already been completed...", category='error')
...
def order_complete(id):
    ...
    if orders.driver_id == partner_id:
        if orders.status != "completed":
            if orders.payment_status == "pending-payment":
...
def login():
    ...
    if form.validate_on_submit():
        ...
        #     print("user login attempt failed")
        #     print("partner login attempt failed")
        #     return redirect(url_for('login'))
    ...
    if partner_form.validate_on_submit():
        partner_username = PartnerSignup.query.filter_by(username=partner_form.username.data).first()
        partner_mail = PartnerSignup.query.filter_by(email=partner_form.email.data).first()
        partner_pass = PartnerSignup.query.filter_by(password=partner_form.password.data).first()
        ...
        # elif partner_mail is None:
        #     flash("Partner does not exist!", category="error")
        #     print("partner login attempt failed")
        #     return redirect(url_for('login'))
    ...
    return render_template('login.html', form=form, partner_form=partner_form)
...
@app.route("/rate/rider/<int:id>/<rate>", methods=["POST", "GET"])
@login_required_user
def rate_rider(id, rate):
    order = Orders.query.get(id)
    if order.user_id != current_user.id:
        print(f"{current_user.id} attempted an illegal rating")
        flash("Error, you are not allowed to perform this action...", category="error")
        return redirect(url_for("user_dash"))
        ...
        if user_details.first_uuid == None:
            ...
            try:
                create_wallet = MyWallet(my_balance=default,
                                            ...
                                            trans_status="Wallet created",
                                            trans_message="create wallet")
